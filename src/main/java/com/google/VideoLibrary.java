...
class VideoLibrary {
  ...
  List<Video> getVideosAsc(){
    List<Video> allVideos = this.getVideos();
    allVideos.sort(Comparator.comparing(Video::getDesc));
    return allVideos;
  }

  Video getRandomVideo(){
    List<Video> allVideos = this.getVideos();
    int randomNum = ThreadLocalRandom.current().nextInt(0,allVideos.size()); //Is this the standard?
    return allVideos.get(randomNum);
  }
